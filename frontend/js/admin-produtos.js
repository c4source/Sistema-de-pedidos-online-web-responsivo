...
function normalizeProducts(productsList) {
  ...
  return productsList.map(function (product) {
    return {
      ...
      imagem: product.imagem || product.imagem_url || '',
...
function normalizeAvailability(product) {
  var status = String(product.status_disponibilidade || '').toLowerCase();
...
function getNextId() {
  ...
  return Math.max.apply(null, produtos.map(function (product) {
...
function handleEditProduct(productData) {
  ...
  var productIndex = produtos.findIndex(function (product) {
  ...
  productData.id = produtos[productIndex].id;
  produtos[productIndex] = productData;
  saveProducts();
  resetForm();
  renderPage();
}
...
function toggleProductAvailability(productId) {
  ...
  product.disponivel = product.disponivel === false;
  saveProducts();
  renderPage();
}
...
function createProductCard(product) {
  ...
  availabilityButton.className = product.disponivel === false ? 'admin-product-action-button admin-product-action-primary' : 'admin-product-action-button admin-product-action-secondary';
  ...
  availabilityButton.textContent = product.disponivel === false ? 'Reativar' : 'Inativar';
  ...
  actions.appendChild(availabilityButton);
